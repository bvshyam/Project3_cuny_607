...
Now, how do we find the average time someone is arrested? Well, we need to convert the date and time to accurately find this information. As the timestamp is
in the AM/PM format, we have to convert everything to twenty four hour time so that you as the analyst can tell the difference. Otherwise, the data will
just all show a 12 hr format with no distinction. Once this occurs, we can perform calculations on the data.
```{r}
twentyFourHourTime <- as.POSIXct(chicago$Date, format = "%m/%d/%Y %I:%M:%S %p")

mean(twentyFourHourTime)
```
The average time that smeone would be arrested in this sample is on 12/15/15 at 1:59AM.
